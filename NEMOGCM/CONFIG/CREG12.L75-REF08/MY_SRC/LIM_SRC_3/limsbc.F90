...
CONTAINS
   ...
   INTEGER FUNCTION lim_sbc_alloc()
      ...
      !!-------------------------------------------------------------------
      ALLOCATE( soce_0(jpi,jpj) , utau_oce(jpi,jpj) , utau_iceoce(jpi,jpj) ,                      &
         &      sice_0(jpi,jpj) , vtau_oce(jpi,jpj) , vtau_iceoce(jpi,jpj) , tmod_io(jpi,jpj), STAT=lim_sbc_alloc)
         !
      utau_iceoce(:,:)=0.0_wp
      vtau_iceoce(:,:)=0.0_wp
...
!CT lateral melting add }
   ...
   SUBROUTINE lim_sbc_tau( kt , pu_oce, pv_oce )
      ...
      DO jj = 2, jpjm1                                !* update the stress WITHOUT a ice-ocean rotation angle
         ...
         END DO
      END DO
      CALL iom_put( "utau_atmoce", utau_oce )
      CALL iom_put( "vtau_atmoce", vtau_oce )
      CALL iom_put( "utau_iceoce", utau_iceoce )
      CALL iom_put( "vtau_iceoce", vtau_iceoce )
